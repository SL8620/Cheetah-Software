...
/*!
 * Cheetah 3 and Mini Cheetah 所有硬件初始化都是通用的
 */
...
void HardwareBridge::handleInterfaceLCM() 
{
	...
	{
		/* LCM自动解码消息再传给回调函数，回调函数可以自动识别消息类型
		* 回调函数在lcm.handle之前被调用，并非并发执行，都在一个线程中完成
		* 调用lcm.handle很重要，函数会保持阻塞状态直到有任务需要做
		*/
...
/*!
 * 写入堆栈16kB缓冲区，确保堆栈增长时不会出现页面错误
 * molck所有页面都与当前进程相关，防止软件内存溢出
 * 如内存耗尽，机器人程序将被OOM杀死，并留下日志
 */
	...
	switch (msg->requestKind) 
	{
		case (s8)ControlParameterRequestKind::SET_USER_PARAM_BY_NAME: 
		{
			...
			else 
			{
				...
				// 回应
				_parameter_response_lcmt.requestNumber = msg->requestNumber;  // 通知设置已经发生改动
				_parameter_response_lcmt.parameterKind = msg->parameterKind;  // 仅用于打印调试语句
				memcpy(_parameter_response_lcmt.value, msg->value, 64);

				strcpy((char*)_parameter_response_lcmt.name,name.c_str());    // 仅用于打印调试语句
...
{
	// 初始化通信模块，调用基类HardwareBridge::initCommon()函数，通过LCM订阅interface和interface_request两个主题，创建LCM线程
	initCommon();
	// 初始化机器人的spi通信和IMU
	initHardware();
	...
	// 启动spi通信任务
	PeriodicMemberFunction<MiniCheetahHardwareBridge> spiTask(&taskManager, .002, "spi", &MiniCheetahHardwareBridge::runSpi, this);
	spiTask.start();
